...
class DoggyT5(nn.Module):
    ...
    def _build_model(self):
        ...
        self.output_positional_embedding = PositionalEncoder(
            self.hidden_dim, self.dropout, self.output_length, self.device
        )
        ...
        output_layer_norm = nn.LayerNorm(transformer_output_dim)

        self.transformer_decoder = TransformerDecoder(
            transformer_decoder_layer, num_layers=self.num_layers, norm=output_layer_norm
        )

        self.projection = nn.Linear(transformer_output_dim, self.hidden_dim)
